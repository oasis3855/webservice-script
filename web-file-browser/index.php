...
if (isset($_GET['mode']) && $_GET['mode'] === 'logout') {
    ...
    func_logoff_auth(basename($_SERVER['PHP_SELF']), True);
} else {
    // ログイン処理
    if (!func_check_auth(basename($_SERVER['PHP_SELF']), basename($_SERVER['PHP_SELF']), True, 'UTF-8', True)) {
        die;
    }
}
...
function main_func($strThisScriptName)
{
    ...
    if (isset($_GET['target_file']) || isset($_POST['target_file'])) {
        if (isset($_POST['target_file'])) {
            print_file_preview($strThisScriptName, $_POST['target_file']);
        } elseif (isset($_GET['target_file'])) {
            print_file_preview($strThisScriptName, $_GET['target_file']);
        } else {
            print_filelist($strThisScriptName, '/');
        }        // エラーの場合ファイル一覧表示
    } else {
...
function print_filelist($strThisScriptName, $strRelDir)
{
    ...
    // 末尾に '/' を付加する
    if ($strRelDir[strlen($strRelDir) - 1] != '/') {
        $strRelDir = $strRelDir . '/';
    }
    ...
    for ($i = 0; $i < count($arrDir); $i++) {
        $strStepDir = '/';
        for ($j = 1; $j <= $i; $j++) {
            $strStepDir = $strStepDir . $arrDir[$j] . '/';
        }
...
function print_file_preview($strThisScriptName, $strRelFile)
{
    ...
    if ($arrMimeElem[0] == "image") {
        ...
        if ($_POST['size']) {
            $nTargetSize = intval($_POST['size']);
        }
        $strImgTitle = '';
        if ($_POST['title']) {
            $strImgTitle = htmlspecialchars($_POST['title']);
        }
        $strImgAlt = '';
        if ($_POST['alt']) {
            $strImgAlt = htmlspecialchars($_POST['alt']);
        }
        ...
        // Alt属性にファイル名を使う場合の処理
        if (!strlen($strImgAlt) && $_POST['alt_fname'] === 'on') {
            $strImgAlt = $strFileBody;
        }
